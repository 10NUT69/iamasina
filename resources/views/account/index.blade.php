...
<div class="max-w-[1536px] mx-auto mt-10 mb-20 px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        @else
                            <button type="button"
                                    ...
                                    class="min-h-10 px-1.5 py-2 text-xs font-semibold leading-tight bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/40 transition flex items-center justify-center gap-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="hidden h-4 w-4 shrink-0 sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @if($accountTab === 'mesaje')
        ...
        @if($activeConversation)
            ...
            <div class="mb-5 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm dark:border-[#333] dark:bg-[#1E1E1E] sm:p-4">
                <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                    <div class="flex min-w-0 items-center gap-3">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-base font-black text-slate-700 dark:bg-slate-800 dark:text-slate-100">
                    ...
                    <div class="flex flex-wrap gap-2 lg:justify-end">
                        <a href="{{ route('account.index', ['tab' => 'mesaje']) }}"
                           class="inline-flex items-center justify-center rounded-lg border border-gray-200 px-3 py-2 text-xs font-black text-gray-700 transition hover:border-[#C81424] hover:text-[#C81424] dark:border-[#333] dark:text-gray-200">
                            Inbox
                        </a>
                        @if($service)
                            <a href="{{ $service->public_url }}"
                        ...
                        @endif
                        <form method="POST"
                              action="{{ route('messages.destroyConversation', $activeConversation) }}"
                              onsubmit="return confirm('Ștergi această conversație?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                    class="inline-flex items-center justify-center rounded-lg bg-red-50 px-3 py-2 text-xs font-black text-red-600 transition hover:bg-red-100 dark:bg-red-900/20 dark:text-red-300 dark:hover:bg-red-900/35">
                                Șterge
                            </button>
                        </form>
                    </div>
                ...
                @if($service)
                    <a href="{{ $service->public_url }}"
                       class="mt-3 flex gap-3 rounded-xl border border-gray-100 bg-gray-50 p-2 transition hover:border-[#C81424]/40 hover:bg-white dark:border-[#333] dark:bg-[#181818] dark:hover:bg-[#252525]">
                        <img src="{{ $service->main_image_url }}"
                             alt="{{ $service->title }}"
            ...
            <div class="rounded-2xl border border-gray-200 bg-gray-50 p-3 dark:border-[#333] dark:bg-[#181818] sm:p-4">
                <div id="conversationMessages" class="h-[420px] space-y-4 overflow-y-auto pr-1 sm:h-[500px] sm:pr-2" data-last-message-id="{{ $activeConversationMessages->last()?->id ?? 0 }}">
        ...
        @else
            <div class="grid gap-4">
                @foreach($conversations as $conversation)
                    ...
                    <article class="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-[#C81424]/25 hover:shadow-lg dark:border-[#333333] dark:bg-[#1E1E1E]">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('account.index', ['tab' => 'mesaje', 'conversation' => $conversation->id]) }}"
                               class="flex min-w-0 flex-1 items-center gap-3">
                                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#fff4f5] text-sm font-black text-[#C81424] dark:bg-[#2a1013] dark:text-red-200">
                                    {{ strtoupper(substr($other?->name ?? 'U', 0, 1)) }}
                                </div>

                                <div class="min-w-0 flex-1">
                                    <div class="flex min-w-0 items-center gap-2">
                                        <p class="truncate text-sm font-black leading-5 text-gray-900 dark:text-white">{{ $other?->name ?? 'Utilizator' }}</p>
                                        @if($conversation->unread_count)
                                            <span class="inline-flex min-w-5 shrink-0 items-center justify-center rounded-full bg-[#C81424] px-1.5 text-[10px] font-black leading-5 text-white">{{ $conversation->unread_count }}</span>
                                        @endif
                                    </div>
                            ...
                            <div class="flex shrink-0 items-center gap-2">
                                <p class="hidden text-[11px] font-semibold text-gray-400 sm:block">
                                    {{ optional($conversation->last_message_at ?? $conversation->updated_at)->diffForHumans() }}
                                </p>
                                <form method="POST"
                                      action="{{ route('messages.destroyConversation', $conversation) }}"
                                      onsubmit="return confirm('Ștergi această conversație?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center justify-center rounded-lg bg-red-50 px-3 py-2 text-xs font-black leading-none text-red-600 transition hover:bg-red-100 dark:bg-red-900/20 dark:text-red-300 dark:hover:bg-red-900/35">
                                        Șterge
                                    </button>
                                </form>
                            </div>
                        ...
                        <a href="{{ route('account.index', ['tab' => 'mesaje', 'conversation' => $conversation->id]) }}"
                           class="mt-2 block rounded-xl border border-gray-100 bg-gray-50 p-2 transition hover:border-[#C81424]/30 hover:bg-white dark:border-[#333333] dark:bg-[#181818] dark:hover:bg-[#252525]">
                            <p class="mb-2 line-clamp-1 text-sm text-gray-600 dark:text-gray-300">
                                @if($latest)
                            ...
                            @if($service)
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="mt-0.5 line-clamp-1 text-sm font-black leading-5 text-gray-900 dark:text-white">{{ $service->title }}</p>
                                        <p class="text-xs font-bold leading-4 text-gray-600 dark:text-gray-300">
                                            @if($service->price_value)
                                                {{ number_format($service->price_value, 0, ',', '.') }} {{ $service->currency }}
                                            @else
                                                Cere ofertă
                                    ...
                                    <div class="hidden shrink-0 items-center text-xs font-black text-[#C81424] md:flex">
                                        Deschide
                                    </div>
                                </div>
                            @else
                    ...
                    <div>
                        ...
                        <div class="md:w-1/2 pr-0 md:pr-3">
                            ...
                            <div class="relative group">
                                ...
                                <input id="editPassword" type="password" minlength="6" placeholder="Lasă gol dacă nu schimbi"
                                    class="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-300 dark:border-[#404040]
                                           bg-gray-50 dark:bg-[#2C2C2C] text-gray-900 dark:text-white text-sm font-medium
                                           focus:ring-2 focus:ring-[#C81424]/20 focus:border-[#C81424] outline-none transition shadow-sm">
                            </div>
