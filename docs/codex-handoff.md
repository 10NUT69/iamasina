...
- 2026-07-12: Optimized public listing filters, with the observed mobile transmission delay as the primary target. `ServiceController::index()` now selects only card-level service columns, eager-loads only card relations with restricted columns, keeps the existing `whereHas('user')` seller-type filter intact, relies on the existing `SoftDeletes` scope, and adds `id DESC` as a stable secondary order. Added `2026_07_12_160000_add_listing_performance_indexes_to_services_table.php` with base newest-listing and transmission/newest composite indexes. The horizontal listing gallery now uses one shared Alpine `listingGallery` component from `resources/js/components/listing-gallery.js`, renders one active `<img>` per card, preloads the adjacent image before activation, preserves arrow/swipe behavior, and marks only the first card image in each result batch as eager. Added `ListingFilterPerformanceTest`, which verifies dealer filtering, transmission filtering, soft-delete exclusion, stable newest ordering, one image per card, and eager/lazy behavior. Verified PHP lint, `git diff --check`, Vite production build, Blade cache compilation, the new feature test (15 assertions), the Unit suite (7 tests / 22 assertions), direct AJAX responses for unfiltered/manual/automatic/dealer+manual listing URLs, a Node gallery preload/next/swipe check, and mobile DOM output at 390x844. The full suite completed with 37 passing and the same 7 pre-existing failures already documented below (registration authentication expectation, missing SQLite `services` schema for the homepage example, and legacy `/profile` route expectations); `ListingFilterPerformanceTest` passes in the full run. The manual-transmission AJAX response dropped from 570,401 to 306,045 bytes; card HTML now contains 20 images instead of 169, no image templates, one eager image, and 19 lazy images. `EXPLAIN ANALYZE` changed the first-20 transmission lookup from 259 rows plus filesort (~3.01 ms locally) to a reverse lookup on `services_transmission_newest_idx` that stops at 20 rows (~0.15 ms locally); the warmed request uses 12 queries and no eager `users` query. The migration is applied locally as batch 23; production still needs the normal `php artisan migrate --force` and asset build during deployment. The in-app browser rendered the updated mobile DOM but did not execute the local `.test` module bundle, so a real-browser arrow/swipe pass remains advisable after deploy. No image jobs, thumbnail backfills, queue records, environment keys, or secrets were changed.

- 2026-07-08: Improved dealer registration company-name validation. `RegisteredUserController` now has a guest-safe `register.checkCompanyName` JSON endpoint for live checking duplicate `company_name` values on the register form, and server-side registration validation now returns Romanian messages such as `Numele parcului auto este deja folosit.` instead of Laravel's default English message. `resources/views/auth/register.blade.php` now displays live availability feedback and suggestions under the dealer company-name field, matching the account profile behavior. The listing-publish flow also received Romanian duplicate/required dealer company-name messages so the same error does not reappear when creating a dealer during ad publishing. Verified PHP lint on touched files, `php artisan route:list --name=register`, `php artisan view:cache`, local `/register` rendering the live-check elements and `/register/check-company-name`, live endpoint returning `available:false` plus suggestions for existing `FD Automobile`, duplicate dealer registration submit rendering the Romanian error and not `The company name has already been taken`, and `php artisan view:clear`. No migrations, database changes, environment keys, secrets, or dealer slug changes were added.

- 2026-07-07: Simplified dealer portfolio canonical configuration after review: removed the extra canonical environment-key idea and now `User::dealer_canonical_url` uses the existing `APP_URL` / `config('app.url')`. Local can stay `APP_URL=http://localhost`, production should stay `APP_URL=https://iaauto.ro`; no additional environment key is needed. `.env.example` and `config/app.php` were reverted to the normal single-URL setup while keeping the dealer legacy 301 behavior, canonical path logic, sitemap canonical dealer URLs, and `Membru fondator` badge changes.

- 2026-07-07: Tightened dealer portfolio SEO/redirect behavior after review. Dealer portfolio canonical URLs, legacy 301 targets, and sitemap dealer URLs now use `User::dealer_canonical_url`, which is based on the existing `APP_URL`; internal links can still use `dealer_public_url` for the current app host. `User` now also exposes `dealer_public_path` so `ServiceController::showDealerPortfolio()` redirects legacy routes unconditionally and redirects any wrong dealer path by path comparison, preventing the legacy `/anunturi-auto-de-vanzare/parc-auto/...` route from ever returning 200. `/parcuri-auto` JSON-LD dealer item URLs now use canonical dealer URLs. The dealer portfolio founding badge now displays `Membru fondator` with the same styling used on the show page. Verified all 7 active local dealer portfolios: new `/parcuri-auto/...` paths return 200 locally, canonical tags use the configured `APP_URL`, old `/anunturi-auto-de-vanzare/parc-auto/...` paths return 301 to the corresponding configured canonical URL, no old route returns 200, all founding dealer pages show `Membru fondator` and not raw `Fondator`, and the sitemap contains all 7 canonical dealer URLs with no old dealer pattern. Also ran PHP lint on touched files, `php artisan config:clear`, `php artisan view:cache`, and `php artisan view:clear`. No migrations, database changes, extra environment keys, secrets, or dealer slug changes were added.

- 2026-07-07: Moved public dealer portfolio canonical URLs from `/anunturi-auto-de-vanzare/parc-auto/{countySlug}/{citySlug}/{dealerSlug}` to `/parcuri-auto/{countySlug}/{citySlug}/{dealerSlug}` while keeping the old route as a 301 legacy redirect. `dealers.show` now points to the new route and the old route is named `dealers.show.legacy`, defined before the general listing routes. `User::dealer_public_url` now generates the new URL, prefers `county_id`/`locality_id` slugs when available, and cleans text fallback values like `Floresti, Cluj` to `floresti` without changing database data or existing `dealer_slug` values. Dealer card queries and sitemap generation eager-load the dealer location relations needed by the accessor. The dealer portfolio page now emits an explicit canonical to the new URL and its breadcrumb is `Acasa -> Parcuri auto -> Judet -> Localitate -> Nume dealer`. Sitemap cache keys were bumped to `sitemap.xml:v5`. Verified PHP lint for touched PHP/Blade files, `php artisan route:list --name=dealers`, `php artisan view:cache`, `php artisan view:clear`, local `/parcuri-auto` HTTP 200, `/parcuri-auto/cluj/floresti/fd-automobile` HTTP 200 with canonical on the new URL, old `/anunturi-auto-de-vanzare/parc-auto/cluj/floresti-cluj/fd-automobile` HTTP 301 to the new URL, homepage/list-dealers/show-page links using the new dealer URL, sitemap containing the new FD URL and not the old dealer URL, rollback-wrapped dealer account creation and individual-to-dealer switch generating new dealer URLs, and `git diff --check`. No migrations, database changes, redirects for `/dealeri-auto`, environment keys, or secrets were added.

- 2026-07-07: Renamed the public all-dealers page URL from `/dealeri-auto` to `/parcuri-auto` without adding a redirect because the page is not live yet. The route name remains `dealers.index`, so homepage CTA links, dealer portfolio breadcrumbs, canonical URL, JSON-LD URL/`@id`, and sitemap entry now resolve to `/parcuri-auto`. `SitemapController` cache keys were bumped to `sitemap.xml:v4` to avoid serving a cached sitemap containing the old URL. Verified with PHP lint on `routes/web.php` and `SitemapController.php`, `php artisan route:list --name=dealers`, `php artisan view:cache`, `php artisan view:clear`, local `/parcuri-auto` HTTP 200 with canonical/schema on the new URL, local `/dealeri-auto` HTTP 404, homepage containing `/parcuri-auto` and not `/dealeri-auto`, sitemap containing `/parcuri-auto` and not `/dealeri-auto`, and `git diff --check`. No migrations, database changes, environment keys, or secrets were added.

- 2026-07-07: Completed the dedicated `/dealeri-auto` content and SEO layer without changing dealer selection/order or card structure. `ServiceController::dealers()` now also passes the total active listing count for dealer accounts. `resources/views/services/dealers.blade.php` now has the requested H1, intro copy, dealer/listing stats, canonical tag, updated meta title/description, `CollectionPage` JSON-LD with an `ItemList` of the visible dealers as `AutoDealer` items, the SEO text block after the dealer grid, and the dealer CTA linking to `services.create`. `resources/views/services/partials/dealer_card.blade.php` now formats card location display as normal title case `Oraș, Județ` and removes the visible `județul` prefix; database values are not modified. Verified with PHP lint on `ServiceController.php`, `dealers.blade.php`, and `dealer_card.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and local `/dealeri-auto` HTTP 200 confirming the canonical, meta title, SEO section, CTA, header stats (`7 dealeri auto`, `37 anunțuri active`), rendered dealer cards, and `CollectionPage` JSON-LD. No migrations, routes, filters, dealer query ordering changes, environment keys, or secrets were added.

- 2026-07-07: Adjusted the homepage dealer carousel mobile layout so the first mobile viewport shows four dealer cards in a 2-column by 2-row block. `resources/views/services/index.blade.php` now uses a two-row horizontal CSS grid for the dealer carousel below tablet widths, while switching back to the existing one-row flex carousel from 768px up for 4 desktop cards and 5 wide-desktop cards. Verified with `php -l resources/views/services/index.blade.php`, `php artisan view:cache`, `php artisan view:clear`, local `/` HTTP 200, rendered CSS checks for `grid-template-rows: repeat(2, minmax(0, 1fr))` and two mobile grid columns, and `git diff --check`. No routes, controllers, migrations, database changes, environment keys, or secrets were added.

- 2026-07-07: Refined the reusable dealer cards after UX feedback. `resources/views/services/partials/dealer_card.blade.php` is now a compact rectangular, fully clickable card with logo/initials and dealer name on the first row, smaller city/county text below, and a red arrow action instead of visible `Vezi stocul`, active stock count, or `Dealer Fondator` badge. The homepage dealer section title is now simply `Dealeri auto`, with the `Vezi toți dealerii auto` CTA centered under the carousel and styled like the existing recent-listings CTA. Homepage carousel sizing now shows 2 cards on mobile, 4 on medium desktop widths, and 5 on wide desktop. `/dealeri-auto` uses the same compact card and grid sizing of 2 mobile, 4 desktop, and 5 wide desktop cards; its visible H1 is now `Dealeri auto`. Verified with PHP lint on the touched Blade files, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and local HTTP 200 checks for `/` and `/dealeri-auto` confirming the old visible card labels are gone and the arrow/card layout renders. No migrations, database changes, routes, environment keys, or secrets were added.

- 2026-07-07: Adjusted the reusable dealer card logo rendering in `resources/views/services/partials/dealer_card.blade.php` so uploaded dealer logos are displayed as contained artwork on a white logo tile instead of cover-cropped like photos. This makes wide or transparent logos visible in the homepage dealer carousel and `/dealeri-auto` cards while preserving initials for dealers without `dealer_logo`. Verified with PHP lint on the partial, `php artisan view:cache`, `php artisan view:clear`, and a local homepage render confirming the dealer card logo image now uses `object-contain`. Local data check showed `B&G AUTO HOUSE` currently has `dealer_logo = null`, so its initials fallback is expected; `CNCD AUTO INVEST SRL` has a saved logo and renders the image in the card. No migrations, database changes, routes, environment keys, or secrets were added.

- 2026-07-07: Added public dealer discovery surfaces. `ServiceController::index()` now loads a small featured dealer set for the homepage using one shared query that keeps only dealers with active listings, counts active stock with `withCount`, orders founding dealers first, then by stock size, and selects only card-level user columns. Added `ServiceController::dealers()` and the `/dealeri-auto` route for a paginated all-dealers page, plus `resources/views/services/partials/dealer_card.blade.php` for reusable dealer cards with logo/initial fallback, location, active listing count, founding badge, and stock link. The homepage now shows the dealer section after the recent-listings CTA with a horizontal carousel sized for 2 cards on mobile and 6 on desktop; `resources/views/services/dealers.blade.php` reuses the same card partial in a paginated grid. The dealer portfolio breadcrumb now points to `/dealeri-auto`, the layout treats `dealers.index` as a tight public page, and sitemap generation now includes the new page with a bumped cache key. Verified with PHP lint on touched PHP/Blade files, `php artisan route:list --name=dealers`, `php artisan view:cache`, `php artisan view:clear`, local HTTP 200 checks for `/`, `/dealeri-auto`, and `/sitemap.xml`, sitemap containing `/dealeri-auto`, `git diff --check`, and an in-app browser responsive check confirming the homepage dealer carousel section is present and uses 1/6 desktop and 1/2 mobile card sizing. Local database currently has only one active eligible dealer, so carousel overflow arrows are naturally hidden locally. No migrations, database changes, environment keys, or secrets were added.

- 2026-07-04: Repaired the GA4 `listing_published` initialization order. `resources/views/components/cookie-consent.blade.php` now marks Analytics ready only after `gtag('config', analyticsId)` runs, then dispatches the internal `iaauto:analytics-ready` event. `resources/views/layouts/app.blade.php` now sends `listing_published` only once, only with Analytics consent, only after `window.iaAutoAnalytics.isReady()` is true, and includes `send_to: analyticsId` so the event is routed to `G-16NX753L0M`. Verified with `php artisan view:cache`, `php artisan view:clear`, `git diff --check -- resources/views/components/cookie-consent.blade.php resources/views/layouts/app.blade.php`, and a rendered Blade check confirming `iaauto:analytics-ready`, `gtag('config', analyticsId)`, and `gtag('event', 'listing_published', { send_to: analyticsId })` are present. Final production/browser validation still needs Tag Assistant and Network confirmation for a `collect` request with `en=listing_published` and destination `G-16NX753L0M`. No controllers, routes, migrations, database changes, environment keys, or secrets were added.

- 2026-07-04: Adjusted the homepage SEO text block under `ANUNȚURI AUTO ÎN ROMÂNIA` in `resources/views/services/index.blade.php` so the heading and paragraph text use a single full-width column (`max-w-none`) and the paragraphs are justified (`text-justify`). The three benefit cards below that text were left unchanged. Verified with `php artisan view:cache`, `php artisan view:clear`, `git diff --check -- resources/views/services/index.blade.php` (only the existing CRLF/LF warning was reported), and a local render of `/` on `auto.test` returning HTTP 200. No controllers, routes, migrations, database changes, environment keys, or secrets were added.

- 2026-07-04: Adjusted account/dealer name validation after the QA audit. Personal `users.name` is now treated as a display name: `ProfileController::ajaxUpdate()` no longer requires it to be unique, and the legacy name-check endpoints return available for personal names. The visible name-availability feedback was removed from the account and register forms so users are not told normal personal names are unavailable. Dealer `company_name` remains the public identity guard: profile updates still validate uniqueness, and the same uniqueness rule was added to dealer registration plus public listing submission when it creates a dealer account. Verified with PHP lint on `ProfileController.php`, `RegisteredUserController.php`, and `ServiceController.php`, `php artisan view:cache`, `php artisan view:clear`, and local HTTP checks on `auto.test`: duplicate personal name `Ionut` saved successfully in profile, `/profile/check-name` returned available, duplicate dealer company name was rejected in profile, `/profile/check-company-name` returned unavailable with suggestions, duplicate dealer company name was rejected during `/register`, and duplicate dealer company name was rejected during listing submission without creating a service or user.

- 2026-07-04: Applied the existing local pending migration `2026_07_03_120000_add_dealer_logo_to_users_table` so the local `users.dealer_logo` column is now available. Verified the dealer logo flow on the local `auto.test` host with a QA seller account: profile update to dealer returned HTTP 200, `/profile/dealer-logo` upload returned HTTP 200 with a stored `dealers/{user_id}/...webp` path, the logo file existed before delete, `/profile/dealer-logo` delete returned HTTP 200 and removed the file/path, then the account was downgraded back to individual. `2026_07_03_130000_add_dealer_tier_to_users_table` remains pending locally.

- 2026-07-04: Extended the favorite-state N+1 fix from show-page related listings to the other public card pages. `ServiceController::index()` now adds preloaded `is_favorited_by_current_user` state to the homepage and listing collections after `count()` and before `get()`, and `showDealerPortfolio()` does the same for dealer portfolio listing cards. The horizontal listing card partial and the older vertical `service_cards` partial now use the preloaded favorite attribute when present, matching `service_cards_home`. Verified with PHP lint on `ServiceController.php`, `service_cards_home.blade.php`, `service_cards_horizontal.blade.php`, and `service_cards.blade.php`, plus `php artisan view:cache`, `php artisan view:clear`, and `git diff --check`. Re-measured public pages as guest/auth: homepage went from 12 per-card favorite queries to 1 embedded favorite-state query when authenticated, listing went from 20 to 1, and dealer portfolio went from per-card favorite queries to 1; with two simulated favorite rows on each page, the count stayed at 1. Simulated favorite rows were inserted only inside rolled-back transactions. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-04: Removed the authenticated-user N+1 favorite checks from the show-page related listings. The shared vertical card partial now uses a preloaded `is_favorited_by_current_user` attribute when present, falling back to `Service::isFavoritedBy()` for other pages that have not precomputed it. The related-services query adds `withExists()` only for authenticated users, so favorite state is included in the main related query rather than one `exists()` query per card. Verified with PHP lint on `ServiceController.php`, `show.blade.php`, and `service_cards_home.blade.php`, `php artisan view:cache`, `php artisan view:clear`, and `git diff --check`. Re-measured show page ID 66: guest returned 25 queries / 0 favorite queries, authenticated user returned 26 queries with favorite state embedded in the main related query, and authenticated user with two simulated favorite related listings still returned 26 queries; the old per-card favorite queries are gone. Test favorite rows were inserted only inside a rolled-back transaction.

- 2026-07-04: Optimized the public show-page related listings query. `ServiceController::similarServicesForShow()` now uses one prioritized query instead of locality/county/country query chaining: same brand/model, active listings only, with real images only (`JSON_LENGTH(images) > 0`), current county first, then the rest of Romania, capped at `RELATED_SERVICES_LIMIT` / 12. The related-card eager loading was reduced to the relations the shared vertical card actually needs (`county`, `locality`, `combustibil`, `cutieViteze`, `brandRel`, `modelRel`) with selected columns only; `user`, `category`, and `normaPoluare` were removed from the related-card load. Legacy `brand`/`model` fallback checks are now schema-aware so local databases without those old columns do not break. Verified with PHP lint on `ServiceController.php`, `show.blade.php`, and `service_cards_home.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, isolated measurement of service ID 66 related listings dropping to 7 queries / 3 cards (`281,218,134`), and a warmed full render of show page ID 66 returning HTTP 200 with 25 total queries. `EXPLAIN` on the main related query still uses `services_model_id_foreign` and `Using filesort`, so a composite index remains the likely next scale optimization once production volume warrants it. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-04: Refactored the homepage vertical listing cards to reuse `resources/views/services/partials/service_cards_home.blade.php` instead of keeping a duplicated inline card block in `resources/views/services/index.blade.php`. The shared partial now supports an `index` variant so the homepage keeps its larger mobile typography/icons, while the show-page related carousel continues to use the compact default with `data-service-card` for carousel sizing. No controllers, routes, publish/edit/account flows, migrations, database changes, environment keys, or secrets were changed. Verified with PHP lint on the touched Blade files, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, local render of `/`, local render of show page ID 66, local render of `/anunturi-auto-de-vanzare/adauga-anunt`, and authenticated local renders of `/contul-meu` plus `/anunturi-auto-de-vanzare/66/edit` as the listing owner; all returned HTTP 200.

- 2026-07-04: Adjusted the public show page related-listings carousel so mobile keeps one full-width related card instead of switching to two cramped cards at 480px/640px. The carousel now stays at one card below the tablet breakpoint, switches to two cards from 768px, and keeps four visible cards from 1024px desktop widths. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, and `git diff --check`. No migrations, routes, controllers, database changes, environment keys, or secrets were added.

- 2026-07-04: Reverted the user-entered service titles back to their neutral color on the public show page because the red title treatment was too visually aggressive. The main user title and the desktop sticky-card title now use `text-gray-900 dark:text-white` again; price styling was left separate. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 confirming both title classes are neutral. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-04: Aligned the user-entered service title color with the price color on both mobile and desktop. The main title and the desktop sticky-card title now use the same red text treatment as the visible price, and the desktop price amount/currency were updated to the same color so the title/price pairing is consistent across breakpoints. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 confirming the main title, desktop title, and desktop price all include `text-[#E03E2D] dark:text-red-300`. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Fixed the related-listings carousel so desktop cannot squeeze all cards into one row. The show-page related carousel now uses a flex track with fixed `flex-basis` values instead of implicit CSS grid columns: 1 full card on narrow mobile, 2 cards from 480px/tablet widths, and exactly 4 visible cards on desktop with overflow for the rest. Carousel arrows are now enabled on mobile too, still controlled by scroll position so the right arrow appears initially when there is overflow and the left arrow appears after scrolling. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and Chrome/Playwright measurements on service ID 66: desktop card basis `calc(25% - 18px)` / no overflow for 3 local cards, mobile 390px shows 1 card with right arrow visible, and after tapping right the left arrow becomes visible. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Tightened the related-listings carousel responsive sizing on the public service show page. The carousel now shows one full related card on narrow mobile widths, two cards from 480px upward when there is enough room, two cards on tablet, and exactly four visible cards from the desktop breakpoint while keeping horizontal scroll for additional results. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 confirming the one/two/four-card CSS rules and 3 related cards rendered. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Added related listings to the public service show page. `ServiceController::showCar()` now builds up to 12 related active listings by prioritizing the same brand/model (or brand-only fallback) in the same locality, then same county, then countrywide, excluding the current listing and deduplicating results. The section title is dynamic (`Alte <marca> <model>` or `Alte <marca>`) and is hidden when no related listings exist. `resources/views/services/show.blade.php` reuses the index/home listing cards in a horizontal scroll-snap carousel, shows four cards per desktop viewport, uses natural horizontal scroll on smaller screens, and desktop arrows appear only when the carousel can scroll: right arrow initially, left arrow after scrolling, with right hidden at the end. Verified with `php -l` on `ServiceController.php` and `show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `php artisan test --testsuite=Unit`, `git diff --check`, and a local render of service ID 66 showing `Alte Nissan Qashqai` with 3 related cards. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Updated the public service show user-entered title color to align with the listing price treatment. The main user title now uses the mobile price red on small screens and switches back to the same neutral strong color as the desktop sticky-card price from `md` upward; the desktop sticky-card title already matched the desktop price color. Verified with `php -l resources/views/services/show.blade.php`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 confirming the main user title has `text-[#E03E2D] md:text-gray-900 md:dark:text-white`. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Adjusted the generated show-page first row to use middle-dot separators instead of the previous comma/year phrasing. The visible H1 and Product/Car JSON-LD `name` now render as `Marca Model Caroserie · An · Norma poluare`, while the compact specs line remains `Motorizare: ...` without repeating the pollution standard. Verified with PHP lint on `ServiceShowHeading.php` and `ServiceShowHeadingTest.php`, `php artisan test --filter=ServiceShowHeadingTest`, `php artisan test --testsuite=Unit`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 showing H1/schema name `Nissan Qashqai SUV · 2020 · Euro 6`. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Refined the public service show generated heading/spec line. The visible H1 now includes the pollution standard on the first row (`Marca Model Caroserie An, Euro 6`), while the compact specs line starts with `Motorizare:` before the engine/fuel fragment and no longer repeats the pollution standard. The same helper behavior applies to both the currently visible desktop/mobile layout and the explicit `ServiceShowHeading::desktop()` variant. The Product/Car JSON-LD `name` continues to follow the visible H1. Verified with PHP lint on the touched show-heading files, `php artisan test --testsuite=Unit`, `php artisan view:cache`, `php artisan view:clear`, `git diff --check`, and a local render of service ID 66 showing H1 `Nissan Qashqai SUV 2020, Euro 6`, specs `Motorizare: 1,3 benzina · 159 CP · Manuala · 192.000 km · Râmnicu Vâlcea, Vâlcea`, and matching schema `name`. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Unified the public service show dynamic H1 across desktop and mobile. `ServiceShowHeading::make()` now returns the short visible H1 (`Marca Model Caroserie An`), `ServiceController::showCar()` passes that value as `showHeading`, and `resources/views/services/show.blade.php` renders one H1 plus the compact specs line on all breakpoints instead of showing the long sentence on desktop. The expanded Product/Car JSON-LD `name` now follows the same visible H1, while the detailed vehicle facts remain in schema properties and the specs line. Verified with PHP lint on `ServiceShowHeading`, `ServiceController`, `show.blade.php`, and `ServiceShowHeadingTest`, `php artisan view:cache`, `php artisan view:clear`, `php artisan test --testsuite=Unit`, `git diff --check`, and a local render of service ID 66 where the H1 and schema name both rendered as `Nissan Qashqai SUV 2020` and the specs line was not desktop-hidden. No migrations, routes, database changes, environment keys, or secrets were added.

- 2026-07-03: Added public listing page JSON-LD for collection context. `resources/views/services/listing.blade.php` now emits a single `@graph` containing `BreadcrumbList`, `CollectionPage`, and an `ItemList` of the visible listing URLs on the current page, while keeping detailed `Product`/`Car`/`Offer` schema only on show pages. The global `WebSite` schema now has `@id: https://iaauto.ro/#website` so listing pages can reference it via `isPartOf`. Verified with `php -l` on the touched Blade files, `php artisan view:cache`, `php artisan view:clear`, `php artisan test --testsuite=Unit`, `git diff --check`, and a local render of `/anunturi-auto-de-vanzare` that produced `WebSite` plus a graph containing `BreadcrumbList`, `CollectionPage`, and `ItemList` with 20 items. No migrations, routes, controllers, database changes, or environment keys were added; production URLs should still be checked in Rich Results Test after deploy.

- 2026-07-03: Refined the expanded service show JSON-LD before publishing: schema `description` now uses the full generated description instead of the 160-character meta snippet, meta description truncation now stops at a word boundary, color combines base color and finish such as `Alb perlat`, engine displacement now includes `unitCode: CMQ` with `unitText: cm³`, and engine power now includes `unitCode: N12` with `unitText: CP`. `priceValidUntil` remains intentionally omitted.

- 2026-07-03: Expanded the public service show page vehicle JSON-LD. The main entity now uses `@type: ["Product", "Car"]`, keeps the separate layout `WebSite` schema and existing `BreadcrumbList`, uses the generated show H1 for `name`, adds `@id`, `mainEntityOfPage`, `sku`, all real `services.images` URLs, vehicle facts from already eager-loaded relations, deduplicated active feature / important detail `additionalProperty` entries, and an `Offer` with price/currency/availability/location/seller. The schema intentionally omits VIN, phone, email, coordinates, `itemCondition`, and `priceValidUntil`; no migrations, database changes, routes, or environment keys were added.
...
- Ran `php -l resources/views/services/show.blade.php`; no syntax errors after the JSON-LD final refinements.
- Ran `php artisan view:cache` and `php artisan view:clear`; Blade templates compiled and cache was cleared after the JSON-LD final refinements.
- Rendered service ID 66 at `http://127.0.0.1:8011/...`; confirmed the product JSON-LD description ends cleanly with `iaAuto.ro.`, color is `Alb perlat`, engine displacement includes `unitCode: CMQ` / `unitText: cm³`, engine power includes `unitCode: N12` / `unitText: CP`, image count is still 8, and `priceValidUntil` is absent.
- Ran `php artisan test --testsuite=Unit`; 7 tests / 20 assertions passed after the JSON-LD final refinements.
- Ran `git diff --check`; passed after the JSON-LD final refinements.

- Ran `php -l resources/views/services/show.blade.php`; no syntax errors after expanding the show-page JSON-LD.
- Ran `php artisan view:cache` and `php artisan view:clear`; Blade templates compiled and cache was cleared after the JSON-LD update.
- Rendered service ID 66 at `http://127.0.0.1:8011/...` and extracted three JSON-LD scripts: the layout `WebSite`, the updated `Product`/`Car` entity, and the existing `BreadcrumbList`.
- Verified the rendered ID 66 product schema has 8 image URLs from `services.images`, `@type` values `Product,Car`, seller type `Person`, and no `priceValidUntil`; checked the compact product JSON for forbidden VIN/phone/email/coordinate/`itemCondition`/`priceValidUntil` keys.
- Ran `php artisan test --testsuite=Unit`; 7 tests / 20 assertions passed after the JSON-LD update.
- Ran `git diff --check`; passed after the JSON-LD update.
...
- The expanded show-page JSON-LD is PHP/Blade-only and uses data already available on the rendered service show page plus relations already eager-loaded in `ServiceController::showCar()`. It does not add migrations, routes, environment keys, secrets, VIN, phone, email, coordinates, `itemCondition`, or `priceValidUntil`. Local verification used a temporary `php artisan serve` instance on `127.0.0.1:8011`.
...
- After deploy, run the live service show URL through Schema.org Validator / Rich Results Test to confirm the rendered `Product`/`Car`, `WebSite`, and `BreadcrumbList` scripts are parsed as expected on the production domain.
